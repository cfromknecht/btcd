...
func (b *BlockChain) calcSequenceLock(node *blockNode, tx *btcutil.Tx, utxoView *UtxoViewpoint, mempool bool) (*SequenceLock, error) {
	...
	for txInIndex, txIn := range mTx.TxIn {
		...
		case sequenceNum&wire.SequenceLockTimeIsSeconds == wire.SequenceLockTimeIsSeconds:
			...
			medianTime := b.index.CalcPastMedianTime(blockNode)
...
func (b *BlockChain) getReorganizeNodes(node *blockNode) (*list.List, *list.List) {
	...
	if b.index.NodeStatus(node.parent).KnownInvalid() {
		b.index.SetStatusFlags(node.self, statusInvalidAncestor)
	...
	invalidChain := false
	for n := node; n != nil && n != forkNode; n = b.index.Parent(n) {
		if b.index.NodeStatus(n.self).KnownInvalid() {
			invalidChain = true
			break
		}
	...
	if invalidChain {
		...
		for e := attachNodes.Front(); e != nil; e = next {
			...
			b.index.SetStatusFlags(n.self, statusInvalidAncestor)
		}
		return detachNodes, attachNodes
	}
	...
	// the main chain.
	for n := b.bestChain.Tip(); n != nil && n != forkNode; n = b.index.Parent(n) {
...
func (b *BlockChain) connectBlock(node *blockNode, block *btcutil.Block,
	...
	state := newBestState(node, blockSize, blockWeight, numTxns,
		curTotalTxns+numTxns, b.index.CalcPastMedianTime(node))
...
func (b *BlockChain) disconnectBlock(node *blockNode, block *btcutil.Block, view *UtxoViewpoint) error {
	...
	state := newBestState(prevNode, blockSize, blockWeight, numTxns,
		newTotalTxns, b.index.CalcPastMedianTime(prevNode))
	...
	b.bestChain.SetTip(b.index.Parent(node))
...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List) error {
	...
	if attachNodes.Len() != 0 && detachNodes.Len() != 0 {
		...
		firstAttachParent := b.index.Parent(firstAttachNode)
		lastDetachParent := b.index.Parent(lastDetachNode)
		if firstAttachParent.hash != lastDetachParent.hash {
			return AssertError(fmt.Sprintf("reorganize nodes do not have the "+
				"same fork point -- first attach parent %v, last detach "+
				"parent %v", &firstAttachParent.hash,
				&lastDetachParent.hash))
		}
	}
	...
	for e := attachNodes.Front(); e != nil; e = e.Next() {
		...
		// view.
		if b.index.NodeStatus(n.self).KnownValid() {
			err = view.fetchInputUtxos(b.db, block)
			if err != nil {
				return err
			}
		...
		if err != nil {
			if _, ok := err.(RuleError); ok {
				b.index.SetStatusFlags(
					n.self, statusValidateFailed,
				)
				for de := e.Next(); de != nil; de = de.Next() {
					dn := de.Value.(*blockNode)
					b.index.SetStatusFlags(
						dn.self, statusInvalidAncestor,
					)
				}
			}
			return err
		}
		b.index.SetStatusFlags(n.self, statusValid)
...
func (b *BlockChain) connectBestChain(node *blockNode, block *btcutil.Block, flags BehaviorFlags) (bool, error) {
	...
	if parentHash.IsEqual(&b.bestChain.Tip().hash) {
		...
		fastAdd = fastAdd || b.index.NodeStatus(node.self).KnownValid()
		...
		if !fastAdd {
			...
			if err == nil {
				...
				b.index.SetStatusFlags(
					node.self, statusValidateFailed,
				)
			} else {
				return false, err
			}
...
func (b *BlockChain) HeightToHashRange(startHeight int32,
	...
	if !b.index.NodeStatus(endNode.self).KnownValid() {
...
func New(config *Config) (*BlockChain, error) {
	...
	b := BlockChain{
		...
		bestChain:           newChainView(nil, index),
