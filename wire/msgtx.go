...
func (c scriptFreeList) maybeBorrow(b *scriptSlab) *scriptSlab {
	if b != nil {
		return b
	}
	return c.Borrow()
...
func (c scriptFreeList) maybeReturn(b, buf *scriptSlab) {
	if b != nil {
		return
	}
	c.Return(buf)
...
func (msg *MsgTx) BtcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	return msg.btcDecodeBuf(r, pver, enc, nil, nil)
}
...
func (msg *MsgTx) btcDecodeBuf(r io.Reader, pver uint32, enc MessageEncoding, b []byte, s *scriptSlab) error {
	...
	scriptBuf := scriptPool.maybeBorrow(s)
	sbuf := scriptBuf[:]
	...
	for i := uint64(0); i < count; i++ {
		...
		if err != nil {
			binarySerializer.maybeReturn(b, buf)
			scriptPool.maybeReturn(s, scriptBuf)
			return err
		}
...
//
// NOTE: b MUST either be nil or at least an 8-byte slice.
func readOutPointBuf(r io.Reader, pver uint32, version int32, op *OutPoint, b []byte) error {
...
//
// NOTE: b MUST either be nil or at least an 8-byte slice.
func writeOutPointBuf(w io.Writer, pver uint32, version int32, op *OutPoint, b []byte) error {
...
//
// NOTE: b MUST either be nil or at least an 8-byte slice.
func readScriptBuf(r io.Reader, pver uint32, b, s []byte, maxAllowed uint32, fieldName string) ([]byte, error) {
	buf := binarySerializer.maybeBorrow(b)
	count, err := ReadVarIntBuf(r, pver, buf)
	binarySerializer.maybeReturn(b, buf)
	if err != nil {
...
//
// NOTE: b MUST either be nil or at least an 8-byte slice.
func readTxInBuf(r io.Reader, pver uint32, version int32, ti *TxIn, b, s []byte) error {
	buf := binarySerializer.maybeBorrow(b)
...
func writeTxInBuf(w io.Writer, pver uint32, version int32, ti *TxIn, b []byte) error {
	buf := binarySerializer.maybeBorrow(b)
...
func readTxOutBuf(r io.Reader, pver uint32, version int32, to *TxOut, b, s []byte) error {
	buf := binarySerializer.maybeBorrow(b)
...
// binarySerializer's pool and return when the method finishes.
func writeTxWitnessBuf(w io.Writer, pver uint32, version int32, wit [][]byte, b []byte) error {
	buf := binarySerializer.maybeBorrow(b)
