...
func readNetAddress(r io.Reader, pver uint32, na *NetAddress, ts bool) error {
	return readNetAddressBuf(r, pver, na, ts, nil)
}
...
func readNetAddressBuf(r io.Reader, pver uint32, na *NetAddress, ts bool, b []byte) error {
	var (
		timestamp time.Time
		services  ServiceFlag
		ip        [16]byte
...
func writeNetAddress(w io.Writer, pver uint32, na *NetAddress, ts bool) error {
	return writeNetAddressBuf(w, pver, na, ts, nil)
}
...
//
// NOTE: b MUST either be nil or at least an 8-byte slice.
func writeNetAddressBuf(w io.Writer, pver uint32, na *NetAddress, ts bool, b []byte) error {
	// NOTE: The bitcoin protocol uses a uint32 for the timestamp so it will
	// stop working somewhere around 2106.  Also timestamp wasn't added until
